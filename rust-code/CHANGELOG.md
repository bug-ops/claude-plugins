...
- `rust-architect`, `rust-critic`, `rust-security-maintenance`, `rust-arch-analyst`, `rust-security-analyst`: model `claude-opus-5` → `claude-opus-5-5`, keeping `effort: high` explicit because the model defaults to `medium`. (#9)
- `continuous-improvement` subagents engine and `sdd-integration` references: in auto mode agents deliver their report through `SubagentHandback`, not a plain final message. (#9)
- `rust-agent-handoff`: names where the handoff block goes: `SendMessage` as a teammate, `SubagentHandback` when available, otherwise the final message. (#9)
- `team-develop` Step 6.5: handles `/code-review` running inline in the desktop app (`ReportFindings`) as well as forked. (#9)
- `team-develop`, `team-debug`: never pass `isolation` on teammate spawns, since it turns them into plain subagents outside the team. (#9)
- Marketplace entry description and keywords synced with `plugin.json`, because `/plugin` → Installed now shows the marketplace entry. (#9)
- `CLAUDE.md`: documents `omitClaudeMd`, `initialPrompt` (ignored for plugin agents) and full-ID model pinning. (#9)
- README: headless `continuous-improvement` runs recommend Claude Code 2.1.280 or later. (#9)

### Fixed

- `continuous-improvement` preflight showed an empty "Last cycle journal" instead of `none` when the journal directory was missing. (#9)
- Root README and `continuous-improvement` still described `rust-arch-analyst` and `rust-security-analyst` as `sonnet` agents. (#9)
- Version badges, plugin and marketplace manifests bumped to `1.47.0`. (#9)
...
Removes the tool and turn restrictions that limited the agents beyond their specialization, and widens the security and architecture audit protocols.
